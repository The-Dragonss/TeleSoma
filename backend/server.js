...
4. **Exhaustive content**: The summarized content should be explained in simpler, understandable terms and be as exhaustive as possible.
...
app.post("/api/openai", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!textContent.trim()) {
      return res
        .status(400)
        .json({ error: "Uploaded file is empty or unreadable." });
    }
    ...
    const openaiResponse = await axios.post(
      "https://api.openai.com/v1/chat/completions",
      {
        ...
        messages: [
          {
            role: "system",
            content: "You are an assistant that formats responses in JSON.",
          },
